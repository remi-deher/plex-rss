<template>
  <section class="panel span-two">
    <div class="panel-head">
      <div><span class="eyebrow">Tendance</span><h2>Activite sur 30 jours</h2></div>
      ...
      <div class="chart-area">
        ...
        >
          ...
          <div
            ...
            role="img"
            :aria-label="`${formatLongDate(timeline.labels[index])} : ${value} ${activeOption.unit}`"
          >
...
const values=computed(()=>props.timeline.series?.[activeSeries.value]||(activeSeries.value==='requests'?props.timeline.values:[])||[]);
...
const peak = computed(() => {
  const points=values.value,labels=props.timeline.labels||[];
  const value=Math.max(0,...points),index=points.indexOf(value);
  return {value,label:index<0?'Aucune activite':formatLongDate(labels[index],{day:'numeric',month:'short'})};
});
...
<style scoped>
.panel-head>div{display:grid;gap:2px}.eyebrow{text-transform:uppercase;letter-spacing:.09em;font-size:10px}.activity-trend{padding:6px 9px;border-radius:999px;border:1px solid var(--border);font-size:11px;font-weight:700}.activity-trend.up{color:var(--success);background:rgba(34,197,94,.08);border-color:rgba(34,197,94,.22)}.activity-trend.down{color:var(--danger);background:rgba(239,68,68,.08);border-color:rgba(239,68,68,.22)}.activity-series{display:flex;gap:6px;margin-top:12px;overflow-x:auto}.activity-series button{display:flex;align-items:center;gap:6px;padding:7px 10px;border:1px solid var(--border);border-radius:999px;background:transparent;color:var(--muted);white-space:nowrap}.activity-series button.active{background:var(--surface-2);color:var(--text);border-color:var(--muted)}.activity-series i{width:7px;height:7px;border-radius:50%;background:var(--accent)}.activity-series .availability i{background:var(--success)}.activity-series .notifications i{background:#8b5cf6}.activity-chart .bar.availability{background:var(--success)}.activity-chart .bar.notifications{background:#8b5cf6}.activity-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:14px}.activity-summary>div{display:grid;grid-template-columns:auto 1fr;align-items:baseline;gap:2px 7px;padding:10px 12px;border:1px solid var(--border);border-radius:9px;background:var(--surface-2)}.activity-summary span{grid-column:1/-1;font-size:10px;text-transform:uppercase;letter-spacing:.06em}.activity-summary strong{font-size:21px;color:var(--text)}.activity-summary small{font-size:10px}@media(max-width:520px){.panel-head{align-items:flex-start}.activity-summary{grid-template-columns:repeat(3,minmax(0,1fr))}.activity-summary>div{display:flex;flex-direction:column;align-items:flex-start;padding:9px}.activity-summary strong{font-size:18px}.activity-chart{height:190px}}
</style>
