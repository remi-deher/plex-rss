  ...
  <div class="page calendar-page">
    ...
    <div v-if="view==='month'" class="month-calendar">
      ...
      <div v-for="cell in monthCells" :key="cell.key" class="month-cell" :class="{outside:!cell.current,today:cell.date===todayStr}">
        ...
        <button v-for="event in cell.events.slice(0,3)" :key="eventKey(event)" class="month-event" :class="eventState(event)" :title="`${event.title} — ${event.subtitle}`" @click="openDetail(event)"><span>{{ formatTime(event.date) }}</span>{{ event.title }}</button>
...
<style scoped>
.calendar-navigation,.calendar-command-bar,.calendar-legend{display:flex;align-items:center;gap:8px}.calendar-command-bar{align-items:stretch}.calendar-command-bar :deep(.ui-filter-bar){flex:1}.calendar-view-switch button{gap:6px;min-width:92px}.calendar-view-switch svg{width:15px}.calendar-legend{justify-content:flex-end;color:var(--muted);font-size:11px}.calendar-legend span{display:flex;align-items:center;gap:5px}.calendar-legend i{width:7px;height:7px;border-radius:50%;background:var(--muted)}.calendar-legend i.available{background:var(--success)}.calendar-legend i.tracked{background:var(--accent)}.month-calendar{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border:1px solid var(--border);border-radius:12px;overflow:hidden;background:var(--surface)}.month-weekday{padding:8px;text-align:center;border-bottom:1px solid var(--border);color:var(--muted);font-size:10px;font-weight:700;text-transform:uppercase}.month-cell{min-height:132px;padding:8px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);background:rgba(255,255,255,.008)}.month-cell:nth-child(7n){border-right:0}.month-cell.outside{opacity:.35}.month-cell.today{background:rgba(229,160,13,.06);box-shadow:inset 0 0 0 1px rgba(229,160,13,.3)}.month-cell header{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}.month-cell header>span{color:var(--text);font-weight:700}.month-cell header small{color:var(--accent);font-size:8px}.month-event,.month-more{display:block;width:100%;margin:3px 0;padding:4px 5px;border:0;border-left:2px solid var(--muted);border-radius:3px;background:rgba(255,255,255,.035);color:var(--text);font-size:9px;text-align:left;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}.month-event.available{border-color:var(--success)}.month-event.pending{border-color:var(--accent)}.month-event span{margin-right:3px;font-size:8px}.month-more{border:0;background:transparent;color:var(--accent)}.today-badge{display:inline-flex;margin-left:7px;padding:3px 6px;border-radius:999px;background:rgba(229,160,13,.12);font-size:9px}@media(max-width:800px){.month-cell{min-height:105px;padding:5px}.month-event{font-size:0}.month-event span{font-size:8px}.calendar-command-bar{display:grid;grid-template-columns:1fr}.calendar-view-switch{justify-self:start}.calendar-legend{justify-content:flex-start}}@media(max-width:640px){.calendar-navigation{width:100%;justify-content:space-between}.calendar-command-bar{position:sticky;top:8px;z-index:20;padding:8px;border:1px solid var(--border);border-radius:10px;background:var(--surface)}.calendar-command-bar :deep(.ui-filter-bar){position:static;padding:0;border:0;background:transparent}.calendar-view-switch{width:100%}.calendar-view-switch button{flex:1}.month-calendar{display:none}}
